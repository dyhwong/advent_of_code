...
fn part1() -> i32 {
    match find_sum(TOTAL) {
        None => panic!("Did not find 2 values adding up to {}", TOTAL),
        Some((entry1, entry2)) => entry1 * entry2,
    }
}

fn part2() -> i32 {
    for &entry in ENTRIES.iter() {
        match find_sum(TOTAL - entry) {
            None => continue,
            Some((entry1, entry2)) => return entry * entry1 * entry2,
        }
    }

    panic!("Did not find 3 values adding up to {}", TOTAL);
}

fn find_sum(total: i32) -> Option<(i32, i32)> {
    // Return 2 numbers that add up to the given total.
    let mut entry_set = HashSet::new();
    for &entry in ENTRIES.iter() {
        let complement = total - entry;
        if !entry_set.contains(&complement) {
            entry_set.insert(entry);
            continue;
        }

        return Some((entry, complement));
    }
